...
if (len(sys.argv) <= 1):
    fn = "hp3458abx_hp34420a_tcsweep_k262_102_1mvdc_nplc100_k2002ltc_1801_test4_oct2024.dsv"   # Hardcoded filename
else:
...
start_sample = 100                          # Start sample for analysis
end_sample   = 54500                        # End sample for analysis
datespan = 8                                # Horizontal timescale resolution , hours/div

enable_detrending = True  # Apply data linear detrending if set to True
min_temp = 18                               # Minimum temperature for scale
max_temp = 37                               # Maximum temperature for scale
...
# Reference value for relative calculations
ref_val1   = 0.001
ref_val2   = 0.001
ref_val3   = 0.001
ref_val4   = 0.001
...
title_label = r'DUT: Hewlett-Packard 34420A US36000874, NPLC100, CH1 & CH2' # µA 
...
axis4_y_label = "DUT1 Deviation relative to temperature, µ%s/%s" % (stats_units, stats_units)
axis5_y_label = "DUT2 Deviation relative to temperature, µ%s/%s" % (stats_units, stats_units)
axis6_y_label = "Reference channels, µ%s/%s of nominal" % (stats_units, stats_units)

chart_name  = ("DUT TC sweep, µ%s/%s to nominal %g %s" % (stats_units, stats_units, round(ref_val5, 3), stats_units) )
chart2_name = "Airbath parameters, DUT temperature"
chart3_name = "Environment conditions"
chart4_name = "CH1 Temperature coefficient relative to airbath"
chart5_name = "CH2 Temperature coefficient relative to airbath"
chart6_name = "Reference from Keithley 262 nVDIV, µ%s/%s to nominal %g %s" % (stats_units, stats_units, round(ref_val5, 3), stats_units)
...
ax_y_resolution  = 50
ax6_y_resolution = 100

dataset_name1  =  "HP 3458A (Reference 1) + %.1f ppm" % ofs1
dataset_name2  =  "HP 3458B (Reference 2) + %.1f ppm" % ofs2
dataset_name3  =  "HP 3459 X9D (Reference 3) + %.1f ppm" % ofs3
dataset_name4  =  "HP 34420A, INL fit"
dataset_name5  =  "HP 34420A CH1"
dataset_name6  =  "HP 34420A CH2"
...
ax_legend_labels = [dataset_name5, dataset_name5 + ", detrended", dataset_name6, dataset_name6 + ", detrended"]
ax6_legend_labels = [dataset_name1, dataset_name2, dataset_name3]
ax2_legend_labels = ['DUT Temperature, °C', 'NTC in test air-bath, °C', 'Programmed temperature, °C']
...
with open(fn) as csvfile:
    ...
    for row in spamreader:
        ...
        if (start_sample < count_cut < end_sample):
            ...
            count = count + 1
            cntr.extend([count])
            val1.extend([ ofs1 + ((float(row[9]) / ref_val1) - 1) * 1e6 ])   # vbl1 : 3458A
            val2.extend([ ofs2 + ((float(row[10]) / ref_val2) - 1) * 1e6 ])  # vbl2 : 3458B
            val3.extend([ ofs3 + ((float(row[11]) / ref_val3) - 1) * 1e6 ])  # vbl3 : 3459X
            val4.extend([ ofs4 + ((float(row[12]) / ref_val4) - 1) * 1e6 ])  # vbl4 : na
            #val5.extend([ ofs5 + (( rs / float(row[4]) / ref_val5) - 1) * 1e6 ])  # vbl5 : 34420-ch1
            val5.extend([ ofs5 + ((float(row[13]) / ref_val5) - 1) * 1e6 ])  # vbl5 : 34420-ch1
            val6.extend([ ofs6 + ((float(row[14]) / ref_val6) - 1) * 1e6 ])  # vbl6 : 34420-ch2
            val7.extend([ ofs7 + ((float(row[15]) / ref_val7) - 1) * 1e6 ])  # vbl7 : 
            val8.extend([ ofs8 + ((float(row[16]) / ref_val8) - 1) * 1e6 ])  # vbl8 :
            
            raw1.extend([ float(row[9])  ])   # vbl1 : 3458A
            raw2.extend([ float(row[10]) ])  # vbl2 : 3458B
            raw3.extend([ float(row[11]) ])  # vbl3 : 3459X
            raw4.extend([ float(row[12]) ])  # vbl4 : na
            raw5.extend([ float(row[13]) ])  # vbl5 : 34420-ch1
            raw6.extend([ float(row[14]) ])  # vbl6 : 34420-ch2
            raw7.extend([ float(row[15]) ])  # vbl7 : 
            raw8.extend([ float(row[16]) ])  # vbl8 : 
            ...
            tmp119.extend([ float(row[29]) ]) # ch1
            
            #chdiff.extend([ (((float(row[10]) - 10000.76) / 10000.76) * 1e3 + 23) - (float(row[29]) - 0.4) ]) # ch-ch difference nV
            chdiff.extend([ (float(row[13]) - float(row[14]) ) * 1e6 ]) # ch-ch difference nV
...
print ("\033[31;2m-d- Median RAW for val1 = %.9f %s" % (np.median(raw1), stats_units) )
print ("\033[32;2m-d- Median RAW for val2 = %.9f %s" % (np.median(raw2), stats_units) )
print ("\033[33;2m-d- Median RAW for val3 = %.9f %s" % (np.median(raw3), stats_units) )
print ("\033[34;2m-d- Median RAW for val4 = %.9f %s" % (np.median(raw4), stats_units) )
print ("\033[35;2m-d- Median RAW for val5 = %.9f, max = %.9f, min = %.9f, %s" % (np.median(raw5), np.max(raw5), np.min(raw5), stats_units) )
print ("\033[36;2m-d- Median RAW for val6 = %.9f %s" % (np.median(raw6), stats_units) )
...
ax6.plot(stamp, filtered1   ,dataset_color1, alpha=0.8, linewidth=1, marker="*", markersize=4, markerfacecolor=dataset_color1)
ax6.plot(stamp, filtered2   ,dataset_color2, alpha=0.8, linewidth=1, marker="1", markersize=4, markerfacecolor=dataset_color2)
ax6.plot(stamp, filtered3   ,dataset_color3, alpha=0.8, linewidth=1, marker="2", markersize=4, markerfacecolor=dataset_color3)
ax.plot(stamp,filtered6   ,dataset_color6l, alpha=0.8,  linewidth=1, marker=" ", markersize=3, markerfacecolor=dataset_color6l)
ax.plot(stamp,filtered6_dt,dataset_color6 , alpha=0.8,  linewidth=2, marker="p", markersize=3, markerfacecolor=dataset_color6)
...
ax4.text(axis45_x_pos, 0.2,("V23: %9g %s" % (tvalue5, stats_units)), fontsize=12, horizontalalignment='center', verticalalignment='center', transform = ax4.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax4.text(axis45_x_pos, 0.13,("Alpha : %.3f µ%s/%s/K" % (alpha5, stats_units, stats_units)), horizontalalignment='center', verticalalignment='center', transform = ax4.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax4.text(axis45_x_pos, 0.08,("Beta : %.3f µ%s/%s/K^2" % (beta5, stats_units, stats_units)), horizontalalignment='center', verticalalignment='center', transform = ax4.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax4.text(axis45_x_pos, 0.01,("Zero TC: %.1f °C" % (zerotctemp5)), horizontalalignment='center', verticalalignment='bottom', transform = ax4.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax5.text(axis45_x_pos, 0.2,("V23: %9g %s" % (tvalue6, stats_units)), fontsize=12, horizontalalignment='center', verticalalignment='center', transform = ax5.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax5.text(axis45_x_pos, 0.13,("Alpha : %.3f µ%s/%s/K" % (alpha6, stats_units, stats_units)), horizontalalignment='center', verticalalignment='center', transform = ax5.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax5.text(axis45_x_pos, 0.08,("Beta : %.3f µ%s/%s/K^2" % (beta6, stats_units, stats_units)), horizontalalignment='center', verticalalignment='center', transform = ax5.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
ax5.text(axis45_x_pos, 0.01,("Zero TC: %.1f °C" % (zerotctemp6)), horizontalalignment='center', verticalalignment='bottom', transform = ax5.transAxes, bbox=dict(facecolor="#eeeeff", alpha=0.45))
...
ax.yaxis.set_major_locator(plt.MultipleLocator(ax_y_resolution))
...
ax6.yaxis.set_major_locator(plt.MultipleLocator(ax6_y_resolution))

ax2.axis([axis_x_min, axis_x_max, min_temp - 0.5, max_temp + 0.5])
...
ax2.set_ylim(min_temp - 0.5, max_temp + 0.5)
...
ax4.set_xlim(min_temp - 0.5, max_temp + 0.5)
ax4.xaxis.set_major_locator(plt.MultipleLocator(1.0))
ax4.yaxis.set_major_locator(plt.MultipleLocator(ax_y_resolution))

ax5.set_xlim(min_temp - 0.5, max_temp + 0.5)
ax5.xaxis.set_major_locator(plt.MultipleLocator(1.0))
ax5.yaxis.set_major_locator(plt.MultipleLocator(ax_y_resolution))
...
ax.legend(ax_legend_labels  )#loc='lower right')
ax6.legend(ax6_legend_labels, loc='lower right')
...
ax5.legend([dataset_name6, dataset_name6 + " samples", "Polyfit vs temperature"])
ax4.legend([dataset_name5, dataset_name5 + " samples", "Polyfit vs temperature"])
